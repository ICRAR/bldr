...
#     pkg_cfg+="--with-libltdl=external "
     ...
     pkg_cfg+="--with-verbs=/usr "

     if [[ -d "/usr/local/cuda" ]]
     then
#         pkg_cfg+="-with-cuda "
          pkg_cfg+='--with-contrib-vt-flags="--with-cuda-dir=/usr/local/cuda"'
     fi
...
do
     pkg_file="openmpi-$pkg_vers.tar.gz"
     pkg_urls="http://www.open-mpi.org/software/ompi/v1.8/downloads/$pkg_file"
